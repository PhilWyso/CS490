...
function populateQDB(response){
	...
	//----------------------------------------------------------------
		...
		var question_name = document.createTextNode(questionDB[i]['qName']);
...
function addQuestion(id){
	...
	var question_name = questionDB[id]['qName'];
...
function ajaxCallCreateTest(test_name){
 var idList = "";
	...
	var data = 'json_string={"header":"createTest","testName":"'+test_name+'","questionList":"'+idList+'"}'
	var request = new XMLHttpRequest();

	request.open('POST', '../php/createTestRequest.php', true);
...
function ajaxCallQuestionDB(){
	...
	var data = 'json_string={"header":"bankRequest"}';

	var request = new XMLHttpRequest();
	
	request.open('POST', '../php/bankRequest.php', true);
	request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
	request.send(data);
