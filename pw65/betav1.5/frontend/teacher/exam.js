...
function populateQDB(response){
	...
	//----------------------------------------------------------------
		...
		add_td.innerHTML = '<div class="text-center" ><input type="button" value="Add" onClick="addQuestion('+i+')" id="question_to_add_'+i+'"></div>'
			...
			if (flag == true) {
			add_td.innerHTML = '<div class="text-center" ><input type="button" disabled value="Add" onClick="addQuestion('+i+')" id="question_to_add_'+i+'"></div>'
			} else {
			add_td.innerHTML = '<div class="text-center" ><input type="button" value="Add" onClick="addQuestion('+i+')" id="question_to_add_'+i+'"></div>'
			}
		}
...
function addQuestion(id){
	...
	new_div.innerHTML = `<div>						 	
							...
							<div><label id="label_points_`+id+`">Point Worth: </label><input style ="width: 25px" type="text" id="points_`+id+`"></input></div>
...
function ajaxCallCreateTest(test_name){
 	...
 	for(var i in questions_added){
  	...
  	var gradeList = "";
  	for (var i in questions_added){
  		gradeList = gradeList + document.getElementById('points_'+i).value + ",";
  	}



	var data = 'json_string={"header":"createExam","examName":"'+test_name+'","questionList":"'+idList+'","pointList":"'+gradeList+'"}'
